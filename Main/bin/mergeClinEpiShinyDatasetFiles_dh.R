...
if (any(grepl("household", shinyFiles))) {
  ...
  if (nrow(household.file) > 0) {
    ...
    if (!all(names(household.file) %in% c("Household_Id", "Participant_Id", "Observation_Id"))) {
      if ("EUPATH_0015467" %in% names(household.file)) { #IRI for household observation type
...
##### ADDED THIS CODE ######
      if (nrow(household.file) != uniqueN(household.file$Household_Id)) {
        ...
        temp <- temp[is.na(temp$EUPATH_0044122),c("Household_Id", non_long)]
...
####################      
      ...
      if('EUPATH_0044122' %in% names(household.file)) { #IRI for Household study timepoint
...
####CHECK HERE, WHY DOESN'T THIS REMOVE MERGEBYTIMEPOINT FROM THE HOUSEHOLD.FILE??? 
     if('mergyByTimepoint' %in% names(household.file)){
        #household.file$'mergeByTimepoint' <- NULL
        household.file[, c("mergeByTimepoint"):=NULL]
      }
...
##### ADDED THIS CODE ######
    ...
    if (nrow(community.file) != uniqueN(community.file$Community_Id)) {
      ...
      temp <- temp[is.na(temp$EUPATH_0035016),c("Community_Id", non_long)]
    ...
    if ('EUPATH_0035016' %in% names(community.file)) { #IRI for compound study timepoint
...
##### ADDED THIS CODE ######
    masterDataTable <- merge(masterDataTable, community.file, by = mergeByCols, allow.cartesian = TRUE)
##########
